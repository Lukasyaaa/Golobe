...
import { contentPart, navbarFromToNumber, navbarFromToTime, navbarFromToValueTypes } from "../../../../types";
import { useDispatch } from "react-redux";
import { navbarSetFromValueAction, navbarSetToValueAction, navbarSwapActiveAction } from "../../../../store/common/navbarReducer";
...
export const NavbarFromTo : FC<navbarFromToProps> = ({about, id, contentType}) =>{
    ...
    const changeFrom = (e : ChangeEvent<HTMLInputElement>, gap : number) =>{
        if(toValue - Number(e.target.value) >= gap){
            dispatch(navbarSetFromValueAction(Number(e.target.value), about.valueType, id, contentType))
        }else{
            dispatch(navbarSetFromValueAction(toValue - gap, about.valueType, id, contentType))
        }
    };
    const changeTo = (e : ChangeEvent<HTMLInputElement>, gap : number) =>{
        if(Number(e.target.value) - fromValue >= gap){
            dispatch(navbarSetToValueAction(Number(e.target.value), about.valueType, id, contentType))
        }else{
            dispatch(navbarSetToValueAction(fromValue + gap, about.valueType, id, contentType))
        }
    }
