...
import type { NavbarCheckboxes as NavbarCheckboxesType } from "../../../../types";

interface NavbarCheckboxesProps{
    about: NavbarCheckboxesType,
    groupId: number,
...
export const NavbarCheckboxes : FC<NavbarCheckboxesProps> = ({about, groupId, prevItemsCount, state, setState}) => {
    ...
    useEffect(() => {
        ...
        if(innerHTML && containerHTML){
            if(isOpened){
                ...
                if(isShowAll) containerHTML.style.height = (innerHTML.offsetHeight + ((opener.current) ? opener.current.offsetHeight : 0)) + "px" //Для Анимации
    ...
    useEffect(() => {
        ...
        if(containerHTML && innerHTML && listHTML && openerHTML){
            if(isOpened){
                if(isShowAll){
                    containerHTML.style.height = "auto";
                } else {
                    containerHTML.style.height = (innerHTML.offsetHeight + openerHTML.offsetHeight) + "px"; //Для Анимации
                    containerHTML.style.height = (listHTML.offsetHeight + openerHTML.offsetHeight) + "px";
                }
            }
        ...
        >
            ...
            <div className="filter-navbar__value checkboxes-navbar__container" ref={container}>
                                    ...
                                    <input 
                                        ...
                                        onChange={(e) => {
                                            if(!e.currentTarget.checked){
                                                const prevValue = [...state].filter(id => id !== i);
                                                setState(prevValue);
                                            } else {
                                                setState([...state, i])
                                            }
                ...
                </div>
                {
                    ...
                    <button 
                        className="checkboxes-navbar__more" type="button"
                        onClick={() => setIsShowAll(prev => !prev)} ref={opener}
                    >
