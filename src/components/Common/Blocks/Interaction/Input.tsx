...
export const Input: FC<InputProps> = ({about, parentCls, isInMassive, isBigger}) => {
    ...
    useEffect(() => {
        return () => { setError(""); setIsGood(false); }
    }, [])
    
    let HideButton = <Fragment />
    if(isCanHide){
            HideButton = <button className={[
                    ...parentCls.map(cl => "field-" + cl + "__show"), 
                    ...parentCls.map(cl => "show-field-" + cl),
                    "field__show", "show-field"
                ].join(" ")} onClick={toggleIsShow}
            >
            {isShow 
                ? <svg 
                    className={[
                        ...parentCls.map(cl => "show-field-" + cl + "__icon"), "show-field__icon",
                    ].join(" ")} 
                    viewBox="0 0 22.5 15" width="22.5" height="15" fill="none"
                >
    ...
    return(
        <div 
            className={[
                ...
                isBigger ? "bigger" : "",
                subinput === undefined ? "without-subinput" : "",
                isHovered ? "_hovered" : ""
            ].filter(Boolean).join(" ")}
        >
