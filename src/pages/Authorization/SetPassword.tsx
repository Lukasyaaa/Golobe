...
export const SetPassword: FC = () => {
    ...
    const passwordAbout : InputAnother = {
        placeholder: "7789BM6X@@H&$K_", subinput: "Create Password", id: "pasword", 
        isCanHide: true, state: password, setState: setPassword,
        validation: getInputValidation(INPUT_AUTHORIZATION_VALIDATION_TYPE.password) as OneDataInputValidation, 
        anotherValue: null, icon: null
    }
    const confirmAbout : InputConfirm = {
    ...
    const checkinInput = () => {
        ...
        const validationResults = [passwordAbout.validation(password), confirmAbout.validation(confirm, password)];
    ...
    return(
        <main 
            className={[
                "set-password", "authorization-part", error !== "" ? "_error" : ""
            ].filter(Boolean).join(" ")}
        >
