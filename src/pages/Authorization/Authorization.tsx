...
import { Input } from "../../components/Common/Blocks/Interaction/Input";
import { useAppDispatch } from "../../store";
import { userSlice } from "../../store/user";

interface About{
    parentCl: string, heading: string, description: string, 
    fields: (FieldItem<Field<typeof INPUT_AUTHORIZATION_VALIDATION_TYPE>> | 
        FieldsItem<Field<typeof INPUT_AUTHORIZATION_VALIDATION_TYPE>>)[],
    button: string, linkToAnother: {pre: string, main: string, path: string}, or: string, 
    variants: AuthorizationVariant[],
}

interface AuthorizationProps{
    type: objType<typeof AUTHORIZATION_TYPE>
}
export const Authorization: FC<AuthorizationProps> = ({type}) => {
    ...
    const initializeInputs = () : InputState<typeof INPUT_AUTHORIZATION_VALIDATION_TYPE>[] =>  {
        if(type === AUTHORIZATION_TYPE.login){
            return([
                { description: INPUT_AUTHORIZATION_VALIDATION_TYPE.email, value: "" },
                { description: INPUT_AUTHORIZATION_VALIDATION_TYPE.password, value: "" }
            ])
        }
        return([
            { description: INPUT_AUTHORIZATION_VALIDATION_TYPE.firstName, value: "" },
            { description: INPUT_AUTHORIZATION_VALIDATION_TYPE.lastName, value: "" },
            { description: INPUT_AUTHORIZATION_VALIDATION_TYPE.email, value: "" },
            { description: INPUT_AUTHORIZATION_VALIDATION_TYPE.phone, value: "" },
            { description: INPUT_AUTHORIZATION_VALIDATION_TYPE.password, value: "" },
            { description: INPUT_AUTHORIZATION_VALIDATION_TYPE.confirmPassword, value: "", }
        ])
    }
    let [inputs, setInputs] = useState<InputState<typeof INPUT_AUTHORIZATION_VALIDATION_TYPE>[]>(initializeInputs());
    useEffect(() => { setInputs(initializeInputs()); }, [type]);

    const info: About = useMemo(() => {
        if(type === AUTHORIZATION_TYPE.login){
            return({
                parentCl: "login", heading: "Login", description: "Login to access your Golobe account",
                fields: [
                    {
                        value: {
                            subinput: "Email", icon: null,
                            validationType: INPUT_AUTHORIZATION_VALIDATION_TYPE.email, 
                            placeholder: "john.doe@gmail.com", id: "email"
                        }, 
                        isMassive: false
                    },
                    {
                        value: {
                            subinput: "Password", icon: null,
                ...
                button: "Login", linkToAnother: {pre: "Don’t have an account?", main: "Sign up", path: signInPath},
                or: "Or login with", variants: [
                    {
                        icon: {
        ...
        return({
            parentCl: "sign-in", heading: "Sign up", description: "Let’s get you all st up so you can access your personal account.",
            fields: [
                {
                    isMassive: true, 
                    value: [
                        {
                            subinput: "First Name", icon: null,
                            validationType: INPUT_AUTHORIZATION_VALIDATION_TYPE.firstName, 
                            placeholder: "john.doe@gmail.com", id: "first-name"
                        },
                        {
                            subinput: "Last Name", icon: null,
                            validationType: INPUT_AUTHORIZATION_VALIDATION_TYPE.lastName, 
                            placeholder: "john.doe@gmail.com", id: "last-name"
                        }
                    ]
                },
                {
                    ...
                    value: [
                        ...
                        {
                            subinput: "Phone Number", icon: null,
                            validationType: INPUT_AUTHORIZATION_VALIDATION_TYPE.phone, 
                            placeholder: "john.doe@gmail.com", id: "phone"
                        }
                    ]
                },
                {
                ...
                },
                {
                    isMassive: false,
                    value: {
                        subinput: "Confirm Password", icon: null,
                        validationType: INPUT_AUTHORIZATION_VALIDATION_TYPE.confirmPassword, 
                        placeholder: "•••••••••••••••••••••••••", id: "confirm-password"
                    }
                }
            ], button: "Create Account", linkToAnother: {pre: "Already have an account?", main: "Login", path: logInPath},
            or: "Or Sign up with", variants: [
                {
                    icon: {
                        viewbox: {minY: 0, minX: 0, width: 18, height: 18}, width: 24, height: 24, pathes: [
                            {
    ...
    if(type === AUTHORIZATION_TYPE.login){
        checkInputs = () => {
            ...
            if(validationResults.includes("Fill field")){ 
                const notFilledCount = validationResults.filter(item => item === "Fill field").length;
                errors.push("Fill field" + ((notFilledCount > 1) ? "s" : "")); 
            }
            if(validationResults.filter(item => item !== "Fill field").some(item => item.trim() !== "")){ 
                const correctCount = validationResults.filter(item => item !== "Fill field").filter(item => item === "").length;
            ...
            if(!validationResults.some(item => item.trim() !== "")){
                const email = getInputState(FIELD_TYPE.email, inputs);
                const users = JSON.parse(localStorage.getItem("users") as string) as User[];

                const userIndex = users.findIndex(u => u.email.includes(email));

                if (userIndex === -1) {
                    setError("Not exist that Account"); return;
                }

                localStorage.setItem("currentUser", String(userIndex));
                dispatch(userSlice.actions.loadUser(users[userIndex]));
                setError(""); navigate(startPath);
            } else if(error !== errors.join(", ")) {
        ...
        checkInputs = () => {
            ...
            const password = getInputState(INPUT_AUTHORIZATION_VALIDATION_TYPE.password, inputs);
            const validationResults = inputs.map(({description, value}) => 
                (description) === INPUT_AUTHORIZATION_VALIDATION_TYPE.confirmPassword 
                    ? (getInputValidation(description) as TwoDataInputValidation)(value, password) 
                    : (getInputValidation(description) as OneDataInputValidation)(value)
            );
            if(validationResults.includes("Fill field")){ 
                const notFilledCount = validationResults.filter(item => item === "Fill field").length;
            ...
            if(!validationResults.some(item => item.trim() !== "")){
                if(isAgree){
                    const firstName = getInputState(INPUT_AUTHORIZATION_VALIDATION_TYPE.firstName, inputs);
                    const lastName = getInputState(INPUT_AUTHORIZATION_VALIDATION_TYPE.lastName, inputs);
                    const email = getInputState(INPUT_AUTHORIZATION_VALIDATION_TYPE.email, inputs);
                    const phone = getInputState(INPUT_AUTHORIZATION_VALIDATION_TYPE.phone, inputs);

                    const users = JSON.parse(localStorage.getItem("users") as string) as User[];
                    let isHave = users.find(user => 
                        user.email.includes(email) || user.name.firstName === firstName || user.phone == phone
                    ) !== undefined;
                    if(!isHave){
                        const newUser = {
                            name: {firstName, lastName}, email: [email], password, phone, 
                            address: "", birthDay: {day: -1, month: -1, year: -1}, 
                            cards: [], ava: "", banner: "", tickets: [], bookings: [],
                            favourites: {flightsPart: [], hotelsPart: []}
                        }
                        localStorage.setItem("users", JSON.stringify([...JSON.parse(localStorage.getItem("users") as string), newUser]));
                        localStorage.setItem("currentUser", JSON.stringify(users.length));
                        dispatch(userSlice.actions.loadUser(newUser));
                        navigate(startPath);
                    } else {
                        setError("Already exist that Account");
                    }
                }
            } else if(error !== errors.join(", ")) {
                setError(errors.join(", "));
    ...
    const renderInput = (input: Field<typeof INPUT_AUTHORIZATION_VALIDATION_TYPE>, isInMassive: boolean, key: number) => {
        const isConfirm = input.validationType === INPUT_AUTHORIZATION_VALIDATION_TYPE.confirmPassword;
        const isPassword = input.validationType === INPUT_AUTHORIZATION_VALIDATION_TYPE.password;
    
        let aboutPart: AboutOneDataPart | AboutTwoDataPart = {
        ...
        return (
            <Input
                key={key}
                about={{
                    ...input, icon: null, isCanHide: isPassword || isConfirm,
                    state: getInputState(input.validationType, inputs), 
    ...
    return(
        <main 
            className={[
                `${info.parentCl}`, "authorization-page", "authorization-part", error !== "" ? "_error" : ""
            ].filter(Boolean).join(" ")}
        >
            <div className="container">
                <AuthorizationImages parentCls={[info.parentCl, "authorization-page", "authorization-part"]} />
                <div className={`${info.parentCl}__subimages authorization-page__subimages authorization-part__subimages`}>
                    <NavLink className={`${info.parentCl}__logo authorization-page__logo authorization-part__logo`} to={startPath}>
                    ...
                    <h1 className={`${info.parentCl}__heading authorization-page__heading authorization-part__heading`}>{info.heading}</h1>
                    <div className={`${info.parentCl}__description authorization-page__description authorization-part__description`}>
                        {info.description}
                    </div>
                    <div className={`${info.parentCl}__fields authorization-page__fields authorization-part__fields`}>
                        {info.fields.map(({isMassive, value}, i) => 
                            isMassive 
                                ? <div className="sign-in__fields-row" key={i}>{value.map((input, j) => 
                                    renderInput(input, true, j))}
                                </div>
                                : renderInput(value, false, i)
                        )}
                    </div>
                    {type === AUTHORIZATION_TYPE.signIn 
                        ...
                        : <div className="login__row">
                            ...
                            <NavLink className="login__forgot" to={forgotPasswordPath}>Forgot Password</NavLink>
                        </div>
                    }
                    <button 
                        className={`${info.parentCl}__button authorization-page__button authorization-part__button button_green`} 
                        type="button" onClick={checkInputs}
                    >
                        {info.button}
                    </button>
                    <div className={`${info.parentCl}__error authorization-page__error authorization-part__error`}>
                        {error}
                    </div>
